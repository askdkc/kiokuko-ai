...
test('unsupported Enno E2E selectors fail closed', async () => {
  ...
  for (const unsupported of ['unsupported']) {
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        return result.results?.[0]?.status === 'failed' && result.results[0].reason === 'unsupported_client';
      },
    );
  }
});
...
test('OpenCode E2E is not-run until the opt-in flag is set', async () => {
  ...
  const output = await execute(process.execPath, [runner, 'opencode'], { cwd: directory });
...
test('the all-client request reduces to the OpenCode definition only', async () => {
  const directory = await mkdtemp(path.join(tmpdir(), 'kiokuko-enno-e2e-all-'));
  const output = await execute(process.execPath, [runner, 'all'], { cwd: directory });
  const result = JSON.parse(output.stdout) as { results?: Array<{ client?: string; status?: string }> };
  assert.deepEqual(result.results, [{
    client: 'opencode',
    status: 'not-run',
    reason: 'RUN_OPENCODE_E2E=1 is not set',
