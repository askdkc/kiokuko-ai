...
test('sample databases are deterministic and match the complete current migration history', async (t) => {
  ...
  const regenerated = path.join(root, 'regenerated.sqlite');
  await createSampleDatabase(regenerated);
  for (const file of [generated, regenerated]) {
  ...
  assert.deepEqual(await readFile(generated), await readFile(regenerated), 'Independent fixture generations must match');
});
