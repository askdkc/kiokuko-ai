...
const taskRunIndexes = [
  ...
  'idx_task_request_receipts_created_at',
  'nudge_deliveries_run_code_sequence',
  'nudge_deliveries_run_sequence',
  'nudge_deliveries_run_checkpoint',
  'idx_orchestration_jobs_ready',
  'idx_enno_execution_leases_owner',
  'idx_compaction_cycles_session',
  'idx_compaction_post_events_pending',
  'idx_enno_work_unit_resources_key',
] as const;
...
function insertRun(
  ...
  database.prepare(`
    ...
    ) VALUES (?, ?, 'opencode', '1.0.0', NULL, NULL, '1', 'standard', '{}', 'intake', 'Task', NULL, '{}', 0, NULL, ?, NULL, ?, ?)
...
function seedTaskRun(database: ReturnType<typeof openConnection>, workspace = 'workspace-1'): void {
  insertRun(database, 'run-1', workspace);
  insertSession(database, 'session-1', workspace);
  insertIntake(database);
}

test('fresh migration applies the current schema and every task-run table and index', async () => {
  const directory = await temporaryDirectory('task-run-fresh');
  ...
  try {
    ...
    assert.deepEqual(
      taskRunTables.filter((table) => !exists(database, 'table', table)),
      [],
    );
    assert.deepEqual(
    ...
    assert.deepEqual(CURRENT_MIGRATION_VERSIONS, [1, 2, 3, 4]);
    assert.equal(database.prepare('SELECT COUNT(*) AS count FROM schema_migrations').get<{ count: number }>()?.count, 4);
    ...
    assert.throws(
      () => database.prepare(`
        ...
        ) VALUES ('unsupported-client', 'workspace', 'codex', '1', 'minimal', '{}', 'intake', 'Task', '{}', 0, ?, ?, ?)
      `).run(now, now, now),
      /client_kind|CHECK|constraint/iu,
    );
  } finally {
    database.close();
  }
...
test('fresh migration uses immutable revisions and no mutable entry shape', async () => {
  const directory = await temporaryDirectory('task-run-revision-schema');
  const database = openConnection(path.join(directory, 'data.sqlite3'));
  try {
...
test('run_intakes enforces one-to-one links, foreign keys, and workspace consistency', async () => {
  const directory = await temporaryDirectory('task-run-intake-link');
  const database = openConnection(path.join(directory, 'data.sqlite3'));
  try {
    migrateDatabase(database, migrationsDirectory);
    seedTaskRun(database);
...
test('intake feedback enforces exactly one target and idempotency uniqueness', async () => {
  const directory = await temporaryDirectory('task-run-feedback');
  const database = openConnection(path.join(directory, 'data.sqlite3'));
  try {
    migrateDatabase(database, migrationsDirectory);
    seedTaskRun(database);
...
test('task-run foreign keys prevent orphaned child rows', async () => {
  const directory = await temporaryDirectory('task-run-foreign-keys');
  const database = openConnection(path.join(directory, 'data.sqlite3'));
  try {
    migrateDatabase(database, migrationsDirectory);
...
test('idempotency schema has composite uniqueness, bounded hash checks, and no raw key/request columns', async () => {
  const directory = await temporaryDirectory('task-run-idempotency-schema');
  const database = openConnection(path.join(directory, 'data.sqlite3'));
  try {
    migrateDatabase(database, migrationsDirectory);
