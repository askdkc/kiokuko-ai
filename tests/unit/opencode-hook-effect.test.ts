...
test('hook execution never searches a workspace binary or ambient PATH', async () => {
  ...
  const decision = await runKiokukoHook(
    { sessionId: 'session-test', cwd: '/workspace' },
    {
      env: {},
...
test('hook execution rejects symlinks, nonzero exits, and non-exact output', async () => {
  ...
  assert.deepEqual(symlink, { kind: 'skip', reason: 'cli_unavailable' });
  ...
  assert.deepEqual(nonzero, { kind: 'skip', reason: 'hook_failed' });
  ...
  assert.deepEqual(extraKey, { kind: 'skip', reason: 'invalid_response' });
});
