...
export const CHECKPOINT_INTAKE_ERROR_MESSAGE =
  'Checkpoint could not bind to an active run.';
...
export const TASK_ANSWER_CONTRACT_FRAGMENT =
  'Use the exact current question. If question.options is non-null, value must be exactly one returned option. If options is null, provide grounded non-empty text. Answers are advisory enrichment: unresolved questions do not suspend coding. Preserve unresolved fields and evidence boundaries at checkpoint.';

export const CHECKPOINT_CONTRACT_FRAGMENT =
  'For a run-bound checkpoint, `runId` and `outcome` are required, the run must be active, and at least one of memories, feedback, or non-empty evidence must be supplied. outcome alone is an invalid empty checkpoint. Do not invent evidence fields such as checks; use commands and/or tests. Without `runId`, provide at least one memory and omit run-bound fields. An unresolved advisory intake question does not block checkpointing: record the current profile, unresolved items, and evidence boundary. A fresh completed checkpoint additionally requires current memory application decisions and successful required evidence. Other completion reports remain explicitly unverified; failed/cancelled/interrupted runs may terminate. Use task_memory_status, task_memory_review and task_memory_verify or task_memory_evidence. A successful terminal checkpoint is allowed at most once per logical request.';
...
export const CHECKPOINT_RUN_ID_DESCRIPTION =
  'Exact run.runId returned by task_prepare. The run must be active; advisory intake questions may remain unresolved.';
