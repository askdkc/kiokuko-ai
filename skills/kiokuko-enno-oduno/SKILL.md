...
Routine implementation details, focused verifier selection, bounded scope inference, and reasonable defaults do not require confirmation when established from repository evidence. Ask for clarification only when an unresolved assumption could materially change user intent, destructive effects, security or authorization boundaries, public API compatibility, data migration, irreversible effects, or the acceptance target. Continue independent authorized work while that clarification is pending; do not turn it into a general plan-approval gate or bypass a host-required confirmation.
